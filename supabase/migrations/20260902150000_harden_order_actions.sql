-- Keep order actions aligned with the financial lifecycle.
...
BEGIN
  ...
  v_is_supervisor := public.has_role(v_user_id, 'supervisor');
  ...
  IF p_to_status IN ('approved', 'invoiced', 'delivered')
     AND NOT (v_is_admin OR v_is_manager OR v_is_supervisor) THEN
    RAISE EXCEPTION USING
      ERRCODE = '42501',
      MESSAGE = 'STATUS_TRANSITION_NOT_AUTHORIZED';
  END IF;
  ...
  IF p_to_status = 'cancelled'
    ...
    RAISE EXCEPTION USING
      ERRCODE = '22023',
      MESSAGE = 'ORDER_HAS_SETTLED_PAYMENTS';
  END IF;
  ...
  SET status = p_to_status,
      ...
      rejection_reason = CASE
        WHEN p_to_status = 'cancelled'
         AND lower(coalesce(p_reason, '')) LIKE '%rejeit%'
          THEN p_reason
