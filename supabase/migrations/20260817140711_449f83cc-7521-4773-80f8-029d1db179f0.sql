...
DROP POLICY IF EXISTS "Gestores can see and manage team orders" ON public.orders;
CREATE POLICY "Gestores can see and manage team orders"
ON public.orders FOR ALL TO authenticated
USING (public.has_role(auth.uid(), 'gestor_comercial'));

DROP POLICY IF EXISTS "Representatives can see their own orders" ON public.orders;
...
DROP POLICY IF EXISTS "Representatives can insert their own orders" ON public.orders;
...
DROP POLICY IF EXISTS "Representatives can update their own draft orders" ON public.orders;
...
DROP POLICY IF EXISTS "Order history access" ON public.order_history;
