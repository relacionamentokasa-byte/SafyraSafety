-- Make payment settlement and commission release one atomic, idempotent operation.
...
ALTER TABLE public.commissions
  ...
  ADD COLUMN IF NOT EXISTS base_value NUMERIC(15,2),
  ADD COLUMN IF NOT EXISTS commission_rate NUMERIC(7,4),
  ADD COLUMN IF NOT EXISTS commission_value NUMERIC(15,2),
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'commissions' AND column_name = 'value'
  ) THEN
    EXECUTE 'UPDATE public.commissions SET commission_value = COALESCE(commission_value, value) WHERE commission_value IS NULL';
  END IF;
END
$$;

CREATE UNIQUE INDEX IF NOT EXISTS commissions_settlement_key_uidx
  ON public.commissions (settlement_key);
...
REVOKE ALL ON FUNCTION public.mark_order_payment_paid(UUID, NUMERIC) FROM authenticated;
